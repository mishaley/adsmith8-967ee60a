...
const PersonaDisplay: React.FC<PersonaDisplayProps> = ({
  ...
  if (!currentPersona) {
    return (
      <td colSpan={2} className="p-4 text-center text-gray-500 bg-transparent">
  ...
  return (
    <div className="flex bg-transparent">
      ...
      <div className="flex flex-row items-center justify-start pr-4 w-24 bg-transparent">
        <button 
          onClick={goToPrevious}
          disabled={totalPairs <= 1}
          className="h-7 w-7 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 bg-transparent"
        >
