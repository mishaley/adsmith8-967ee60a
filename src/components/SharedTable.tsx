...
function SharedTable<T extends TableName>({
  ...
  return <div className="w-full">
      ...
      gridTemplateColumns: "200px 100px"
        ...
        <div className="bg-[#d3e4fd] p-4 mb-2">
          <Button onClick={handleAdd} className="h-10 w-[80px] rounded-full bg-[#ecb652] text-[14px] text-[#154851] border-2 border-white hover:bg-[#ecb652]/90">
            ADD
          </Button>
        </div>
      </div>
      ...
      gridTemplateColumns: "200px 100px"
        ...
        {mainColumns.map(column => <div key={column.field} className="flex flex-col h-full">
            ...
            <div className="flex-1 bg-white">
              {data.map(row => <div key={row.id} className="p-4 border-b whitespace-nowrap overflow-hidden text-ellipsis">
                  {row[column.field]}
                </div>)}
            </div>
          </div>)}
